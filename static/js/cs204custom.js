...
/* Eventually, we won't need this function, I hope. No, the PRE
 * elements created by Markdown have multiple children, so we'll just
 * ignore this. */
...
// April 30, 2021, switched to padding, since FF was positioning the
// H2 such that the border was exactly at the top of the page, so the
// padding is better.

// Also changed from 100px to 60px, since the nav bar is 60px high. this works in Chrome, FF and Safari

function addMarginTopToFragment() {
    var hash = document.location.hash;
    if( hash ) {
        console.log("adding a padding-top to "+hash);
        $(hash).css('padding-top','60px');
    }
}
